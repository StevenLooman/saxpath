...
function SaXPath(saxParser, xpath, recorder) {
    ...
    this.saxParser.on('opencdata',function() {
        if (typeof self.recorder.onOpenCDATA === 'function') {
            self.recorder.onOpenCDATA();
        }
    });
    this.saxParser.on('closecdata',function() {
        if (typeof self.recorder.onCloseCDATA === 'function') {
            self.recorder.onCloseCDATA();
        }
    });
    this.saxParser.on('cdata',    function(cdata) {
        if (typeof self.recorder.onCDATA === 'function') {
            self.recorder.onCDATA(cdata);
        }
    });
